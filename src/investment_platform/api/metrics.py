...
scheduler_jobs_total = Counter(
    "scheduler_jobs_total", "Total number of scheduler jobs", ["status", "asset_type"]
)
...
scheduler_job_retries_total = Counter(
    "scheduler_job_retries_total", "Total number of job retries", ["job_id", "asset_type"]
)

scheduler_active_jobs = Gauge(
    "scheduler_active_jobs", "Number of active scheduler jobs", ["asset_type"]
)

scheduler_pending_jobs = Gauge(
    "scheduler_pending_jobs", "Number of pending scheduler jobs", ["asset_type"]
)

scheduler_failed_jobs = Gauge(
    "scheduler_failed_jobs", "Number of failed scheduler jobs", ["asset_type"]
)
...
def record_job_execution(
    ...
    scheduler_job_executions_total.labels(
        status=status, asset_type=asset_type, error_category=error_category or "none"
    ).inc()

    scheduler_job_duration_seconds.labels(asset_type=asset_type, status=status).observe(
        duration_seconds
    )

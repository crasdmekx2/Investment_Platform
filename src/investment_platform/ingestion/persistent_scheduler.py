...
class PersistentScheduler(IngestionScheduler):
    ...
    def _load_job_from_row(self, job_row: Dict[str, Any]) -> str:
        ...
        is_execute_now = trigger_config.get("execute_now", False) if isinstance(trigger_config, dict) else False
        if is_execute_now:
            self.logger.info(f"Skipping scheduling for execute_now job {job_id} - it should only be triggered manually")
        ...
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if job_status not in ("active", "pending"):
                    self.logger.info(f"Job {job_id} is not active (status: {job_status}), skipping retry")
                    return
                ...
                delay_seconds = int(retry_delay_seconds * (retry_backoff_multiplier ** current_retry_attempt))
                ...
                try:
                    ...
                    collector_kwargs = json.loads(job_dict["collector_kwargs"]) if job_dict["collector_kwargs"] and isinstance(job_dict["collector_kwargs"], str) else job_dict["collector_kwargs"]
                    asset_metadata = json.loads(job_dict["asset_metadata"]) if job_dict["asset_metadata"] and isinstance(job_dict["asset_metadata"], str) else job_dict["asset_metadata"]
                    ...
                    def retry_job():
                        ...
                        self.logger.info(f"Executing retry {current_retry_attempt + 1} for job {job_id}")
                        
                        # Calculate dates
                        exec_end_date = job_dict["end_date"] if job_dict["end_date"] is not None else datetime.now()
                        exec_start_date = job_dict["start_date"] if job_dict["start_date"] is not None else exec_end_date - timedelta(days=1)
                        ...
                        try:
                            ...
                            if result.get('status') == 'failed':
                                from investment_platform.ingestion.error_classifier import classify_error
                                error_cat, _ = classify_error(Exception(result.get('error_message', '')), result.get('error_message'))
                    ...
                    self.logger.error(f"Failed to schedule retry for job {job_id}: {e}", exc_info=True)
    ...
    def add_job_from_database(self, job_id: str) -> bool:
        ...
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                # Check if this is an execute_now job - these should not be scheduled
                trigger_config = json.loads(job_dict["trigger_config"]) if isinstance(job_dict["trigger_config"], str) else job_dict["trigger_config"]
                is_execute_now = trigger_config.get("execute_now", False) if isinstance(trigger_config, dict) else False
                if is_execute_now:
                    self.logger.info(f"Job {job_id} is execute_now - not adding to scheduler (should be triggered manually)")
        ...
        try:
            with get_db_connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                    ...
                    if job_dict["status"] not in ("active", "pending"):
                        self.logger.warning(f"Job {job_id} has status {job_dict['status']}, cannot trigger")
                        return False
                    ...
                    exec_start_date = start_date if start_date is not None else exec_end_date - timedelta(days=1)
                    ...
                    self.logger.info(f"Manually triggering job {job_id} from database (not in scheduler)")
                    start_time = time.time()
                    ...
                    if error_category is None and execution_status == "failed":
                        ...
                        if error_message:
                            error_category, _ = classify_error(Exception(error_message), error_message)
    ...
    def check_dependencies_met(self, job_id: str) -> tuple[bool, List[str]]:
        ...
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                return (len(unmet) == 0, [d["depends_on_job_id"] for d in dependencies if d["depends_on_job_id"] in unmet])
    ...
    def _add_job_with_dependency_check(
        ...
        def job_with_dependency_check():
            # Check dependencies before executing
            all_met, unmet = self.check_dependencies_met(job_id)
            if not all_met:
                self.logger.info(
                    f"Skipping job {job_id} execution - unmet dependencies: {unmet}"
                )
                return {
                    "asset_id": None,
                    "records_collected": 0,
    ...
    def check_and_trigger_dependent_jobs(self, completed_job_id: str):
        """

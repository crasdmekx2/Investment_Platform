...
class IngestionEngine:
        ...
        try:
            ...
            if self.incremental:
                ...
                if not missing_ranges:
                    self.logger.info(f"All data already exists for {symbol}, skipping collection")
                    result["status"] = "success"
                    result["records_collected"] = 0
                    result["records_loaded"] = 0
                    return result

                self.logger.info(f"Found {len(missing_ranges)} missing range(s) for {symbol}")
            else:
            ...
            for range_start, range_end in missing_ranges:
                self.logger.info(f"Collecting data for {symbol} from {range_start} to {range_end}")
                ...
                mapped_data = self.schema_mapper.map_data(data, asset_type, asset_id)
            ...
            if total_loaded > 0:
                ...
                if total_collected == 0:
                    result["error_message"] = (
                        f"No data collected for {symbol}. The collector returned empty results for the date range {start_date} to {end_date}. This may indicate: 1) The symbol is invalid or not available, 2) No data exists for this date range, 3) API/service returned no data."
                    )
                else:
                    result["error_message"] = (
                        f"Collected {total_collected} records but failed to load any into database. Data may have been invalid or database insertion failed. Check logs for validation errors or constraint violations."
                    )

            # Ensure error_message is set if status is failed
            if result["status"] == "failed" and (
                not result["error_message"] or result["error_message"].strip() == ""
            ):
    ...
    def _get_asset_info(
        ...
        try:
            ...
            self.logger.warning(f"Could not get asset info for {symbol}: {e}. Using defaults.")
            # Return default info
            return {
                "name": symbol,
                "source": "Unknown",

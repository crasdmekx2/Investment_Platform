    ...
    try:
        ...
        try:
            data = collector.collect_historical_data(
                symbol=symbol, start_date=start_date, end_date=end_date, **collect_kwargs
            )
...
def main():
    ...
    # Test StockCollector
    results["StockCollector"] = test_collector("StockCollector", StockCollector, "AAPL")

    # Test ForexCollector
    results["ForexCollector"] = test_collector("ForexCollector", ForexCollector, "USD_EUR")

    # Test CryptoCollector
    results["CryptoCollector"] = test_collector(
        "CryptoCollector", CryptoCollector, "BTC-USD", collect_kwargs={"granularity": "ONE_DAY"}
    )

    # Test BondCollector
    results["BondCollector"] = test_collector("BondCollector", BondCollector, "TBILLS")

    # Test CommodityCollector
    results["CommodityCollector"] = test_collector("CommodityCollector", CommodityCollector, "Gold")

    # Test EconomicCollector (using DGS10 - 10-Year Treasury Rate, daily data)
    results["EconomicCollector"] = test_collector("EconomicCollector", EconomicCollector, "DGS10")

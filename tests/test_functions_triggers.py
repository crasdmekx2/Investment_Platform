...
class TestUpdateUpdatedAtTrigger:
    ...
    def test_trigger_does_not_update_on_insert(self, db_cursor, db_connection):
        ...
        assert (
            abs((created_at - updated_at).total_seconds()) < 1
        ), "created_at and updated_at should be the same on insert"
...
class TestGetAssetBySymbol:
    ...
    def test_get_existing_asset(self, db_cursor, db_connection):
        ...
        asset_id = db_helpers.insert_sample_asset(
            db_cursor, unique_symbol, "stock", "Test Stock", "Test", exchange="NYSE", currency="USD"
        )
        db_connection.commit()
...
class TestGetLatestMarketData:
    ...
    def test_get_latest_data_exists(self, db_cursor, db_connection):
        ...
        result_close = result[
            4
        ]  # close is 5th column (0-indexed: time, open, high, low, close, volume)
        ...
        assert (
            time_diff < 60
        ), f"Function should return most recent time, got {result_time}, expected {expected_time}"
        # Convert Decimal to float for comparison
        result_close_float = (
            float(result_close) if hasattr(result_close, "__float__") else result_close
        )
        assert (
            abs(result_close_float - 107.0) < 0.01
        ), f"Function should return latest close price, got {result_close_float}, expected 107.0"
...
class TestCheckDataFreshness:
    ...
    @pytest.mark.parametrize(
        "data_type", ["market_data", "forex_rates", "bond_rates", "economic_data"]
    )
    ...
    def test_check_freshness_invalid_data_type(self, db_cursor, db_connection):
        ...
        with pytest.raises(Exception) as exc_info:
            db_cursor.execute(
                "SELECT * FROM check_data_freshness(%s, 'invalid_type');", (asset_id,)
            )
            db_cursor.fetchone()

        assert (
            "invalid" in str(exc_info.value).lower() or "exception" in str(exc_info.value).lower()
        )

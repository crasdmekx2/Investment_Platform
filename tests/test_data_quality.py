...
class TestSchemaValidation:
    ...
    def test_assets_column_types(self, db_cursor):
        ...
        assert column_dict['asset_id']['data_type'] in ['integer', 'bigint'], \
            "asset_id should be integer or bigint"
        assert 'character varying' in column_dict['symbol']['data_type'].lower() or \
               'varchar' in column_dict['symbol']['data_type'].lower(), \
            "symbol should be VARCHAR"
        assert column_dict['is_active']['data_type'] == 'boolean', \
            "is_active should be boolean"
        assert 'timestamp' in column_dict['created_at']['data_type'].lower(), \
    ...
    def test_market_data_column_types(self, db_cursor):
        ...
        assert 'timestamp' in column_dict['time']['data_type'].lower(), \
            "time should be TIMESTAMPTZ"
        assert column_dict['asset_id']['data_type'] in ['integer', 'bigint'], \
            "asset_id should be integer or bigint"
        assert 'numeric' in column_dict['open']['data_type'].lower(), \
            "open should be NUMERIC"
        assert 'bigint' in column_dict['volume']['data_type'].lower() or \
               'integer' in column_dict['volume']['data_type'].lower(), \
    ...
    def test_required_fields_not_null(self, db_cursor):
        ...
        for field in required_fields:
            assert column_dict[field]['is_nullable'] == 'NO', \
                f"Field '{field}' should be NOT NULL"
...
class TestDefaultValues:
    ...
    def test_created_at_default(self, db_cursor, db_connection):
        ...
        assert time_diff < 300, f"created_at should be recent (within 5 min), got {time_diff} seconds difference"
...
class TestDataCollectionLogTracking:
    ...
    def test_collection_log_tracks_execution(self, db_cursor, db_connection):
        ...
        assert log_entry[2] == 'TestCollector', "Collector type should match"  # collector_type is index 2
    ...
    def test_collection_log_tracks_failures(self, db_cursor, db_connection):
        ...
        db_cursor.execute("SELECT status, error_message FROM data_collection_log WHERE log_id = %s;", (log_id,))
        result = db_cursor.fetchone()

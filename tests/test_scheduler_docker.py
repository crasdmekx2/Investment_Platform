...
class TestSchedulerDocker:
    ...
    def test_scheduler_connects_to_database(self):
        ...
        if not has_connection:
            ...
            if has_errors:
                pytest.fail(f"Scheduler appears to have database connection issues. Logs: {logs[-500:]}")
    ...
    def test_scheduler_environment_variables(self):
        ...
        result = subprocess.run(
            ["docker", "inspect", "--format", "{{range .Config.Env}}{{println .}}{{end}}", "investment_platform_scheduler"],
            capture_output=True,
            text=True,
            timeout=10,
        )

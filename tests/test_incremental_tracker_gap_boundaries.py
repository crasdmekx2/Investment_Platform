...
class TestIncrementalTrackerGapBoundaries:
    ...
    def test_gap_boundary_uses_microseconds_not_days(self, db_cursor):
        ...
        db_cursor.execute("DELETE FROM market_data WHERE asset_id IN (SELECT asset_id FROM assets WHERE symbol = 'GAP_BOUNDARY_TEST')")
        ...
        missing_ranges = tracker.calculate_missing_ranges(
            asset_id, "stock", start_date, end_date
        )
        ...
        gap_before_start = gap_before[0].replace(tzinfo=None) if gap_before[0].tzinfo else gap_before[0]
        ...
        gap_before_end = gap_before[1].replace(tzinfo=None) if gap_before[1].tzinfo else gap_before[1]
        expected_gap_end_no_tz = expected_gap_end.replace(tzinfo=None) if expected_gap_end.tzinfo else expected_gap_end
        ...
        expected_gap_start_no_tz = expected_gap_start.replace(tzinfo=None) if expected_gap_start.tzinfo else expected_gap_start
        ...
        existing_time_no_tz = existing_time.replace(tzinfo=None) if existing_time.tzinfo else existing_time
    ...
    def test_gap_boundary_with_same_day_data(self, db_cursor):
        ...
        # Clean up
        db_cursor.execute("DELETE FROM market_data WHERE asset_id IN (SELECT asset_id FROM assets WHERE symbol = 'SAME_DAY_GAP')")
        ...
        morning_time_no_tz = morning_time.replace(tzinfo=None) if morning_time.tzinfo else morning_time
